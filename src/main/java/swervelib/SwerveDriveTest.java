...
public class SwerveDriveTest {
    ...
    public static double findDriveMotorKV(
        ...
        for (double kV_new = 0; kV_new < maxVolts; kV_new += 0.0001) {
            ...
            while ((Timer.getFPGATimestamp() - startTimeSeconds) < testDelaySeconds && !foundkV) {
                for (int i = 0; i < modules.length; i++) {
                    if ((modules[i].getDriveMotor().getPosition() - startingEncoders[i])
                            > minMovement) {
                        foundkV = true;
                        break;
                    }
     ...
     * Although SysIdRoutine expects to be logging Voltage, this function logs in Duty-Cycle
     * (percent output) because it results in correctly adjusted values in the analysis for use in
     * this library.
     *
    ...
    public static void logDriveMotorActivity(
        ...
        SmartDashboard.putNumber(
                "Module[" + module.configuration.name + "] SysId Drive Power", power);
        SmartDashboard.putNumber(
    ...
    public static void logAngularMotorActivity(
        ...
        SmartDashboard.putNumber(
                "Module[" + module.configuration.name + "] SysId Angle Power", power);
        SmartDashboard.putNumber(
