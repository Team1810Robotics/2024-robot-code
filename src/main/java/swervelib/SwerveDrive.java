...
public class SwerveDrive {
    ...
    /**
     * Correct chassis velocity in {@link SwerveDrive#drive(Translation2d, double, boolean,
     * boolean)} using 254's correction.
     */
     ...
     * SwerveDrive#drive} method, or via the {@link SwerveDrive#setRawModuleStates} method. The
     * {@link SwerveDrive#drive} method incorporates kinematics-- it takes a translation and
     * rotation, as well as parameters for field-centric and closed-loop velocity control. {@link
     * SwerveDrive#setRawModuleStates} takes a list of SwerveModuleStates and directly passes them
     * to the modules. This subsystem also handles odometry.
     *
                ...
                new SwerveDrivePoseEstimator(
                        ...
                        new Pose2d(
                                ...
                                Rotation2d.fromDegrees(
                                        0))); // x,y,heading in radians; Vision measurement std dev,
        // higher=less weight
    ...
    /**
     * The primary method for controlling the drivebase. Takes a {@link ChassisSpeeds}, and
        ...
        if (headingCorrection) {
            if (Math.abs(velocity.omegaRadiansPerSecond) < HEADING_CORRECTION_DEADBAND
                    && (Math.abs(velocity.vxMetersPerSecond) > HEADING_CORRECTION_DEADBAND
                            || Math.abs(velocity.vyMetersPerSecond)
     ...
     * SwerveDrive#maxSpeedMPS} whichever is higher.
     *
    ...
    /**
     * Set the expected gyroscope angle using a {@link Rotation3d} object. To reset gyro, set to a
     * new {@link Rotation3d} subtracted from the current gyroscopic readings {@link
     ...
     * used for the {@link SwerveDrive#setRawModuleStates(SwerveModuleState[], boolean)} function
     * and {@link SwerveController#getTargetSpeeds(double, double, double, double, double)}
     * functions. This function overrides what was placed in the JSON and could damage your
     ...
     * @param visionMeasurementStdDevs Vision measurement standard deviation that will be sent to
     *     the {@link SwerveDrivePoseEstimator}.The standard deviation of the vision measurement,
     *     for best accuracy calculate the standard deviation at 2 or more points and fit a line to
     *     it with the calculated optimal standard deviation. (Units should be meters per pixel). By
     *     optimizing this you can get * vision accurate to inches instead of feet.
     */
     ...
     * which can be used to generate {@link ChassisSpeeds} for the robot to orient it correctly
     * given axis or angles, and apply {@link edu.wpi.first.math.filter.SlewRateLimiter} to given
     * inputs. Important functions to look at are {@link SwerveController#getTargetSpeeds(double,
     * double, double, double, double)}, {@link
